...
import sys 
import atexit 
from datetime import datetime # Added for the new routes
...
# --- MOCK CONFIGURATION FOR /update_endpoint and status ---
# This configuration simulates settings that might be updated by an admin.
MOCK_CONFIG = {
    "api_version": "v1.0.0",
    "risk_model_version": "LR-v1.0",
    "external_reporting_endpoint": "https://default-vendor-api.com/v1/telematics",
    "last_endpoint_update": time.strftime("%Y-%m-%d %H:%M:%S")
}
...
@app.route('/simulate_telematics', methods=['POST'])
def simulate_telematics():
    """
    Receives raw telematics data points (e.g., from a device simulator) and 
    stores them in the raw_telematics collection for later feature calculation.

    Expected JSON input format (can be a single object or a list of objects):
    {
        "device_id": "DRV-9012",
    ...
    # Handle single item or list of items for batch insertion
    data_list = data if isinstance(data, list) else [data]
    
    required_keys = ['device_id', 'timestamp', 'variable', 'value']
    valid_data_points = []
    
    for item in data_list:
        # Validate that the necessary keys are present
        if all(key in item for key in required_keys):
            # Map keys to match the MongoDB schema defined in the existing code
            ...
            print(f"⚠️ Dropping malformed telematics point: {item}")

    if not valid_data_points:
        return jsonify({"error": "No valid telematics data points provided in the payload."}), 400

    try:
        with get_mongo_client() as client:
            db = client[DATABASE_NAME]
            collection = db[TELEMATICS_COLLECTION]
            
            # Use insert_many for efficiency
            result = collection.insert_many(valid_data_points)
            
            print(f"🎉 Successfully inserted {len(result.inserted_ids)} new telematics points.")

            return jsonify({
                "status": "success",
                "message": f"Successfully recorded {len(result.inserted_ids)} telematics data points.",
                "inserted_count": len(result.inserted_ids)
            }), 200

    except Exception as e:
        print(f"❌ Error inserting telematics data: {e}", file=sys.stderr)
        return jsonify({"status": "error", "message": f"Database error during simulation: {e}"}), 500


@app.route('/update_endpoint', methods=['POST'])
def update_endpoint():
    """
    Updates the external vendor reporting URL in the MOCK_CONFIG. 
    Requires a specific auth_token for simulation security.
    """
    global MOCK_CONFIG
    ...
    # NOTE: Change 'TELEM_ADMIN_123' to a secret environment variable in production.
    if update_data.get('auth_token') != "TELEM_ADMIN_123":
           return jsonify({"error": "Unauthorized: Invalid or missing 'auth_token'"}), 401

    new_url = update_data.get('new_endpoint_url')
    
    if new_url:
        # Update the mock global configuration
        MOCK_CONFIG["external_reporting_endpoint"] = new_url
        MOCK_CONFIG["last_endpoint_update"] = time.strftime("%Y-%m-%d %H:%M:%S")
        
        response_message = f"External reporting endpoint successfully updated to: {new_url}"
        print(response_message)
        
        return jsonify({
            "status": "success",
            "message": response_message,
            "current_endpoint": MOCK_CONFIG["external_reporting_endpoint"]
        }), 200
    else:
        return jsonify({"error": "Missing 'new_endpoint_url' in update data"}), 400

@app.route('/config_status', methods=['GET'])
def config_status():
    """Returns the current operational configuration status."""
